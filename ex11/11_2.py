...
def cells_adjacent(arr, row_array, col_array):
    ...
    cell_adjacent = [(x, y) for x, y in itertools.product(row_adjacent, col_adjacent) if (x, y) != (row_array, col_array)]
...
max_lights = current_data.shape[0] * current_data.shape[1]
lights_counter = 0

step = 0

while lights_counter != max_lights:
    step += 1
    ...
    lights_counter = len(current_data[current_data == 0])

print(' ---- STEP: {} ---- '.format({step}))
print(current_data)
print('Solution step nr:', step)
